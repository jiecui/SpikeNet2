...
def find_optimal_learning_rate_pytorch_lightning(model, train_dataloader, val_dataloader, config):
    """
    ...
    try:
        ...
        if lr_finder is not None:
            ...
            if hasattr(lr_finder, 'suggestion'):
                ...
                suggested_lr = lr_finder.results['lr'][lr_finder.results['loss'].index(min(lr_finder.results['loss']))]
            ...
            if hasattr(lr_finder, 'plot'):
                try:
                    ...
                    if fig is not None:
                        plot_path = os.path.join(get_output_root(), "models", "lr_finder_plot.png")
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description='Train model with PyTorch Lightning Learning Rate Finder')
    parser.add_argument('--lr-finder-only', action='store_true', 
...
for i in range(1):
    ...
    if args.skip_lr_finder:
        ...
        optimal_lr = find_optimal_learning_rate_pytorch_lightning(model, train_dataloader, val_dataloader, config)
        ...
        print(f"🚀 Training with learning rate: {optimal_lr:.2e}")
    ...
    wandb_logger = WandbLogger(project="spikenet2", name=f"spikenet2_lr_{optimal_lr:.2e}")
